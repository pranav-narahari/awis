...
from tracker import Detection, TrackedObject, validate_points
...
def draw_tracked_objects(
    ...
    if id_thickness is None:
        id_thickness = max(2, int(frame_scale / 5))
    if label_size is None:
        label_size = int(max(frame_scale / 100, 1))
    ...
    for obj in objects:
        ...
        if draw_box:
            ...
            lw = 2 or max(int(min(frame.size) / 200), 2)
            ...
            cv2.rectangle(frame, c1, c2, (0,0,0), -1, cv2.LINE_AA)
